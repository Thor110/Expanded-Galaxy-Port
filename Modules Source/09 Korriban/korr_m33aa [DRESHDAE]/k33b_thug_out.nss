	...
	if ((!sub2(intGLOB_55))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub2(intGLOB_52)) {
				...
				if (((nLocal < int4) && (!sub2(intGLOB_52)))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub2(intGLOB_53), 2.5, float3);
			if (sub2(intGLOB_46)) {
				ActionDoCommand(sub5());
			}
			SetLocalNumber(OBJECT_SELF, intGLOB_43, nLocal);
			if ((string2 != "UNKNOWN")) {
				ActionDoCommand(sub1());
			}
		}
	}
}
...
void main() {
	...
	object oKorr_citizenmwlk = GetObjectByTag("korr_citizenmwlk", 0);
	AssignCommand(oKorr_citizenmwlk, sub1());
	...
	SignalEvent(oK33b_siththug1, unknown1);
	SignalEvent(oK33b_siththug2, unknown1);
	SignalEvent(oK33b_siththug3, unknown1);
	ActionMoveToObject(oK33_way_yuthuraexit, 1, 1.0);
