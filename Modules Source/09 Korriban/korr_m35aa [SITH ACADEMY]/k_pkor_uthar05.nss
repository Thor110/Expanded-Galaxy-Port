...
void sub1(int intParam1) {
	SetGlobalNumber("KOR_SITH_HOSTILE", intParam1);
}

void main() {
	AdjustAlignment(GetPCSpeaker(), 3, 10);
	sub1(3);
	object oKor35_yuthura = GetObjectByTag("kor35_yuthura", 0);
	object oPC = GetFirstPC();
	if (GetIsObjectValid(GetItemPossessedBy(oPC, "k35_itm_sithpass"))) {
		DestroyObject(GetItemPossessedBy(oPC, "k35_itm_sithpass"), 0.0, 0, 0.0);
	}
	AssignCommand(oKor35_yuthura, SpeakString("I am dead!", 0));
	SpeakString("I am dead!", 0);
