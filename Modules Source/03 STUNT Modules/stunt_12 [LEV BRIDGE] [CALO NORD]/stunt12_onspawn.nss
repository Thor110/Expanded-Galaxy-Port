			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub13(intGLOB_43, 1));
					}
					else {
	...
	if ((!sub11(intGLOB_52))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub11(intGLOB_49)) {
				...
				if (((nLocal < int4) && (!sub11(intGLOB_49)))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub11(intGLOB_50), 2.5, float3);
			if (sub11(intGLOB_43)) {
...
void main() {
	...
	sub2();
	sub10();
	float float1 = IntToFloat((Random(2) + 1));
	DelayCommand(float1, PlayAnimation(15, 1.0, 999.0));
}
