...
void sub3(int intParam1, object objectParam2) {
	if (((intParam1 == 1) || (intParam1 == 0))) {
		sub4(GetArea(objectParam2), intGLOB_10, intParam1);
	}
	return;
}
...
int sub1(object objectParam1) {
	return sub2(GetArea(objectParam1), intGLOB_10);
}

void main() {
	object oForcefieldloop = GetObjectByTag("forcefieldloop", 0);
	SoundObjectStop(oForcefieldloop);
	if ((sub1(OBJECT_SELF) == 0)) {
		GivePlotXP("kas23_mainwookplot", 5);
		sub3(1, OBJECT_SELF);
	}
}
