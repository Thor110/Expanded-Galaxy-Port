...
void main() {
	...
	if (GetIsPC(oEntering)) {
		if ((sub1(OBJECT_SELF, intGLOB_118) == 0)) {
			RemovePartyMember(0);
			RemovePartyMember(2);
			SetGlobalNumber("DAN_BASTILA_ZONE", 10);
			AddJournalQuestEntry("k_ebonhawk", 1, 0);
			AssignCommand(sub2(), ActionStartConversation(oEntering, "bastila", 0, 0, 1, "", "", "", "", "", "", 0));
			sub3(OBJECT_SELF, intGLOB_118, 1);
		}
		else {
			if (((sub1(OBJECT_SELF, intGLOB_119) == 0) && (GetGlobalNumber("DAN_JEDI_PLOT") == 1))) {
				RemovePartyMember(2);
				object oCarth = GetObjectByTag("carth", 0);
				SetGlobalNumber("DAN_CARTH_ZONE", 4);
				AssignCommand(oCarth, ActionStartConversation(oEntering, "carth", 0, 0, 1, "", "", "", "", "", "", 0));
				sub3(OBJECT_SELF, intGLOB_119, 1);
			}
		}
		sub4();
	}
