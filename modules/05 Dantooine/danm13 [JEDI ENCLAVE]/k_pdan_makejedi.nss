...
AddMultiClass(CLASS_TYPE_JEDIGUARDIAN, GetFirstPC());
			...
			if ((nGlobal == 2)) {
AddMultiClass(CLASS_TYPE_JEDICONSULAR, GetFirstPC());
			}
			else {
				if ((nGlobal == 3)) {
AddMultiClass(CLASS_TYPE_JEDISENTINEL, GetFirstPC());
				}
			}
		}
	}
...
void main() {
	...
	if ((((GetLevelByClass(0, oPC) == 0) && (GetLevelByClass(1, oPC) == 0)) && (GetLevelByClass(2, oPC) == 0))) {
		sub1();
	}
	ClearAllActions();
}
