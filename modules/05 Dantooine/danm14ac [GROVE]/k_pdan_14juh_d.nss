...
// Prototypes
void sub82();
void sub81(object objectParam1);
void sub80(object objectParam1, int intParam2, int intParam3);
int sub79(object objectParam1, int intParam2);
...
void sub82() {
	...
	while ((int1 <= 2)) {
		...
		if ((GetIsObjectValid(oNPC) && (GetFirstPC() != oNPC))) {
			ExecuteScript("k_pman_aion", oNPC, 0xFFFFFFFF);
		}
		(int1++);
	}
}
...
void sub81(object objectParam1) {
	...
	if (GetIsObjectValid(objectParam1)) {
		...
		while ((GetIsEffectValid(efNext) && (!int1))) {
			if ((GetEffectCreator(efNext) == OBJECT_SELF)) {
...
void main() {
	...
	switch (int1) {
		case 10:
			ClearAllActions();
			ActionPlayAnimation(0, 1.0, 0.1);
			ActionMoveToObject(GetObjectByTag("wp_cut01", 0), 0, 1.0);
			ActionDoCommand(SignalEvent(GetObjectByTag("dan14ac_cutobj02", 0), EventUserDefined(110)));
			ActionDoCommand(SignalEvent(GetObjectByTag("dan14ac_cutobj01", 0), EventUserDefined(0)));
			break;
		case 20:
			ActionWait(1.0);
			ActionEquipMostDamagingMelee(OBJECT_INVALID, 0);
		...
		case 40:
			if (GetIsObjectValid(object3)) {
				ActionCastFakeSpellAtObject(29, object3, 0);
				ActionDoCommand(ApplyEffectToObject(2, effect1, object3, 0.0));
				ActionWait(0.5);
				ActionDoCommand(SignalEvent(GetObjectByTag("dan14ac_cutobj01", 0), EventUserDefined(0)));
			}
			break;
		case 80:
			...
			ClearAllActions();
			DelayCommand(0.2, sub1(GetFirstPC()));
			break;
		case 1003:
			if ((sub79(OBJECT_SELF, intGLOB_122) == 0)) {
				ClearAllActions();
				sub1(GetFirstPC());
			}
			break;
		case 1006:
			if (((GetCurrentHitPoints(OBJECT_SELF) < 50) && (sub79(OBJECT_SELF, intGLOB_122) == 0))) {
				sub80(OBJECT_SELF, intGLOB_122, 1);
				sub81(oNPC);
				sub81(object3);
				sub82();
				SetGlobalBoolean("DAN_JUHANI_FIGHT", 0);
				SurrenderToEnemies();
				CancelCombat(GetFirstPC());
				CancelCombat(oNPC);
				CancelCombat(object3);
				NoClicksFor(1.2);
				DelayCommand(0.5, SurrenderToEnemies());
				DelayCommand(1.0, ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", ""));
			}
			break;
		case 1007:
			...
			{
				object oDan13_groveflee = GetObjectByTag("dan13_groveflee", 0);
				if (GetIsObjectValid(oDan13_groveflee)) {
					DestroyObject(oDan13_groveflee, 0.0, 0, 0.0);
				}
			}
			break;
	}
