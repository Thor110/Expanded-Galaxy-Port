...
// Prototypes
void sub19();
int sub18(int intParam1, int intParam2);
void sub17();
	...
	if ((!sub16(intGLOB_55))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub16(intGLOB_52)) {
				...
				if (((nLocal < int4) && (!sub16(intGLOB_52)))) {
					int16 = sub18(int4, nLocal);
					nLocal = (nLocal + int16);
				}
				else {
					if ((nLocal == int4)) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub16(intGLOB_53), 2.5, float3);
			if (sub16(intGLOB_46)) {
				ActionDoCommand(sub19());
			}
			SetLocalNumber(OBJECT_SELF, intGLOB_43, nLocal);
			if ((string2 != "UNKNOWN")) {
				ActionDoCommand(sub17());
			}
		}
	}
}
...
void sub8() {
	...
	if ((sub16(intGLOB_112) || GetIsEncounterCreature(OBJECT_SELF))) {
		string string3;
		int int7 = 1;
		object object1;
		object object2;
...
void sub2(int intParam1) {
	...
	ActionDoCommand(sub2(intParam1));
}
