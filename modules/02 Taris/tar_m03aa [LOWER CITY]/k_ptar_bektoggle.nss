...
void main() {
	...
	int int1 = ((sub1(GetPartyMemberByIndex(0)) || sub1(GetPartyMemberByIndex(1))) || sub1(GetPartyMemberByIndex(2)));
	int1;
	if (int1) {
		if ((int1 || (!GetGlobalBoolean("TAR_BEKBASEOPEN")))) {
			SetLocked(oEntering, 1);
		}
		else {
			SetLocked(oEntering, 0);
		}
	}
}
