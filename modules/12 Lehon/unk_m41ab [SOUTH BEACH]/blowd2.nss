	...
	if (GetIsObjectValid(object1)) {
		...
		while ((GetIsObjectValid(oNearest) && (GetDistanceBetween(oNearest, object1) <= floatParam2))) {
			if (((intParam3 == 1) || GetIsEnemy(oNearest, GetFirstPC()))) {
				efDeath = EffectDamage(int3, 8, 0);
				DelayCommand(float1, ApplyEffectToObject(0, efDeath, oNearest, 0.0));
				efDeath = EffectDeath(0, 1);
				DelayCommand((float1 + 0.1), ApplyEffectToObject(0, efDeath, oNearest, 0.0));
...
void main() {
	...
	if ((GetIsObjectValid(oEntering) == 1)) {
		sub1("object valid", 5, 5, 5.0);
	}
	AssignCommand(GetObjectByTag("bb2", 0), ApplyEffectToObject(0, EffectForcePushed(), oEntering, 0.0));
	sub2("unk41_dpillar2", 10.0, 1);
	sub2("unk41_dpillar3", 10.0, 1);
}
