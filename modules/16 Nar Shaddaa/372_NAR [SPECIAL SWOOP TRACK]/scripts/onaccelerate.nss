int StartingConditional() {
	...
	if ((nGlobal == (-1))) {
		...
		if ((nGlobal == 0)) {
			...
			SWMG_SetPlayerMaxSpeed((float2 + 10.0));
			SWMG_SetPlayerAccelerationPerSecond((float6 * float13));
			vector struct1;
			struct1.x = (-13.0);
			struct1.y = 3.0;
			struct1.z = 0.0;
			SWMG_SetPlayerTunnelPos([6.0,3.0,13.0]);
			...
			if ((nGlobal == 1)) {
				if ((float15 > float2)) {
					...
					SWMG_SetPlayerMaxSpeed((float3 + 10.0));
				...
				if ((nGlobal == 2)) {
					if ((float15 > float3)) {
						...
						SWMG_SetPlayerMaxSpeed((float4 + 5.0));
					...
					if ((nGlobal == 3)) {
						if ((float15 > float4)) {
							...
							SWMG_SetPlayerMaxSpeed((float5 + 5.0));
