...
void main() {
	...
	object oLev40_airlock2 = GetObjectByTag("lev40_airlock2", 0);
	effect efNext;
	if ((GetIsPC(oEntering) && (!sub1(OBJECT_SELF)))) {
		sub2(OBJECT_SELF, 1);
		DestroyObject(GetObjectByTag("lev40_injofficer", 0), 0.0, 0, 0.0);
		SetGlobalBoolean("LEV_SPACEWALKING", 0);
		efNext = GetFirstEffect(GetFirstPC());
		while (GetIsEffectValid(efNext)) {
			if ((GetEffectType(efNext) == 44)) {
				RemoveEffect(GetFirstPC(), efNext);
			}
			efNext = GetNextEffect(GetFirstPC());
		}
		AssignCommand(oLev40_airlock2, ActionCloseDoor(oLev40_airlock2));
	}
}
