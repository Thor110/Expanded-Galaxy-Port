...
void main() {
	...
	int int1;
	SetGlobalNumber("LEV_RIOTS", (GetGlobalNumber("LEV_RIOTS") | 2));
	int1 = 0;
	while ((int1 < 4)) {
		oLev40_starguard = GetObjectByTag("lev40_cell08", int1);
		AssignCommand(oLev40_starguard, ActionOpenDoor(oLev40_starguard));
		(int1++);
	}
	oLev40_starguard = GetObjectByTag("lev40_stardoor", 0);
	AssignCommand(oLev40_starguard, ActionOpenDoor(oLev40_starguard));
	int1 = 0;
	while ((int1 < 5)) {
		oLev40_starguard = GetObjectByTag("lev40_starslave", int1);
		if (GetIsObjectValid(oLev40_starguard)) {
			ChangeToStandardFaction(oLev40_starguard, 4);
			DelayCommand((int1 * 1.0), AssignCommand(oLev40_starguard, sub1(GetObjectByTag("lev40_wpstarslave", int1))));
		}
		oLev40_starguard = GetObjectByTag("lev40_starguard", int1);
		if (GetIsObjectValid(oLev40_starguard)) {
			ChangeToStandardFaction(oLev40_starguard, 2);
			DelayCommand((int1 * 1.0), AssignCommand(oLev40_starguard, sub2(GetObjectByTag("lev40_wpstarguard", int1))));
		}
		(int1++);
	}
