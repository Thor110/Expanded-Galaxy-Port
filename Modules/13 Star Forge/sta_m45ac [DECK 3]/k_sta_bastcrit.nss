...
void main() {
	...
	ApplyEffectToObject(1, EffectBeam(2038, oSta_bastila_heal, 1, 0), OBJECT_SELF, 2.0);
	ApplyEffectToObject(1, EffectVisualEffect(1019, 0), OBJECT_SELF, 3.0);
	object oPC = GetFirstPC();
	ChangeToStandardFaction(OBJECT_SELF, 1);
	ClearAllActions();
	ActionCastSpellAtObject(35, oPC, 0, 0, 0, 0, 0);
	DelayCommand(0.5, sub1(oPC));
	int nGlobal = GetGlobalNumber("Sta_BastConversion");
	sub79(("Bast Conversion = " + IntToString(nGlobal)), 10, 10, 4.0);
}
