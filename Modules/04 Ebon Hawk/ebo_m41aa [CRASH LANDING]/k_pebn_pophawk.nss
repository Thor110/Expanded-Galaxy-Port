...
string sub10(string stringParam1, string stringParam2, int intParam3);
void sub9(string stringParam1, int intParam2, int intParam3, float floatParam4);
void sub8();
void sub7(object objectParam1);
void sub6();
void sub5(int intParam1, location locationParam2);
void sub4();
void sub3();
...
string sub10(string stringParam1, string stringParam2, int intParam3) {
	while ((GetStringLength(stringParam1) < intParam3)) {
		stringParam1 = (stringParam2 + stringParam1);
	}
	return stringParam1;
}
...
void sub8() {
	...
	while (GetIsObjectValid(GetObjectByTag("gizka", int1))) {
		...
		(int1++);
		sub9(("Destroy Gizka " + IntToString(int1)), 5, 5, 1.0);
	}
	int int3;
	int int4 = 0;
	if ((GetGlobalNumber("EBO_GIZKA_PLOT") == 1)) {
		...
		while ((int4 <= int3)) {
			sub9(("Spawn Gizka " + IntToString(int3)), 7, 7, 1.0);
			(int4++);
			CreateObject(1, "g_gizka016", GetLocation(GetObjectByTag(("wp_Gizka_" + sub10(IntToString(int4), "0", 2)), 0)), 0);
		}
	}
	...
	if ((GetLoadFromSaveGame() == 0)) {
		...
		if ((IsAvailableCreature(0) && (GetGlobalBoolean("DAN_BASTILA_AT_JEDI") == 0))) {
			...
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(0, location1));
		...
		if (IsAvailableCreature(1)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(1, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if ((IsAvailableCreature(2) && (GetGlobalBoolean("DAN_CARTH_AT_JEDI") == 0))) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(2, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if (IsAvailableCreature(3)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(3, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if (IsAvailableCreature(4)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(4, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if (IsAvailableCreature(5)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(5, location1));
				DelayCommand(1.0, ClearAllEffects());
				DelayCommand(2.0, SaveNPCState(5));
			}
		}
		if (IsAvailableCreature(6)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(6, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if (IsAvailableCreature(7)) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(7, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if ((IsAvailableCreature(8) && (GetGlobalBoolean("tat_ZaalbarTaken") == 0))) {
			...
			location1 = GetLocation(oPebn_zaalbar);
			if (GetIsObjectValid(oPebn_zaalbar)) {
				DelayCommand(0.5, sub5(8, location1));
				DelayCommand(1.0, ClearAllEffects());
		...
		if (((nGlobal == 5) && (!GetIsObjectValid(oEBO_MysteryBox)))) {
			...
			location location11 = GetLocation(oEBO_MYSTERY_SPAWN);
			DelayCommand(0.5, sub11(64, "g_pebn_mystery", location11));
		}
		else {
