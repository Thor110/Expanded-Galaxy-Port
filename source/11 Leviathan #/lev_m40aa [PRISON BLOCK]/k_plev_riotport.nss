...
void sub2(object objectParam1) {
	ClearAllActions();
	ActionMoveToObject(objectParam1, 1, 1.0);
	ActionDoCommand(sub3(OBJECT_INVALID));
}
...
void main() {
	object oLev40_portguard;
	int int1;
	SetGlobalNumber("LEV_RIOTS", (GetGlobalNumber("LEV_RIOTS") | 1));
	int1 = 0;
	while ((int1 < 4)) {
		oLev40_portguard = GetObjectByTag("lev40_cell09", int1);
		AssignCommand(oLev40_portguard, ActionOpenDoor(oLev40_portguard));
		(int1++);
	}
	oLev40_portguard = GetObjectByTag("lev40_portdoor", 0);
	AssignCommand(oLev40_portguard, ActionOpenDoor(oLev40_portguard));
	int1 = 0;
	while ((int1 < 6)) {
		oLev40_portguard = GetObjectByTag("lev40_portslave", int1);
		if (GetIsObjectValid(oLev40_portguard)) {
			ChangeToStandardFaction(oLev40_portguard, 4);
			DelayCommand((int1 * 1.0), AssignCommand(oLev40_portguard, sub1(GetObjectByTag("lev40_wpportslave", int1))));
		}
		oLev40_portguard = GetObjectByTag("lev40_portguard", int1);
		if (GetIsObjectValid(oLev40_portguard)) {
			ChangeToStandardFaction(oLev40_portguard, 2);
			DelayCommand((int1 * 1.0), AssignCommand(oLev40_portguard, sub2(GetObjectByTag("lev40_wpportguard", int1))));
		}
		(int1++);
	}
