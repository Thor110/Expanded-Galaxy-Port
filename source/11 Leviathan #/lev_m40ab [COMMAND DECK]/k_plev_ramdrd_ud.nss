...
void main() {
	...
	if ((int1 == 1001)) {
		object oLev40_storage = GetObjectByTag("lev40_storage", 0);
		if ((GetDistanceBetween(OBJECT_SELF, oLev40_storage) < 3.0)) {
			ApplyEffectToObject(0, EffectDeath(0, 1), OBJECT_SELF, 0.0);
			AssignCommand(oLev40_storage, ActionOpenDoor(oLev40_storage));
		}
		else {
			sub1(OBJECT_SELF);
		}
