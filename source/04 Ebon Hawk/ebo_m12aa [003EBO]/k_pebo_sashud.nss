...
void main() {
	...
	if ((int1 == 666)) {
		if (((GetGlobalNumber("Ebo_sasha_State") >= 20) && (GetGlobalNumber("Ebo_sasha_State") < 40))) {
			ClearAllActions();
			NoClicksFor(0.4);
			DelayCommand(0.2, ActionStartConversation(GetFirstPC(), "", 0, 0, 1, "", "", "", "", "", "", 0));
		}
	}
	else {
		if ((int1 == 667)) {
			if ((GetIsConversationActive() == 0)) {
				ActionMoveToObject(GetObjectByTag("SashaSpawn", 0), 0, 1.0);
			}
		}
		else {
			if ((int1 == 1001)) {
