...
void main() {
	sub1(OBJECT_SELF, intGLOB_119, 1);
	object oK33_academyguard = GetObjectByTag("k33_academyguard", 0);
	ChangeToStandardFaction(OBJECT_SELF, 2);
	ChangeToStandardFaction(oK33_academyguard, 4);
	object object3 = GetItemPossessedBy(oK33_academyguard, "G_W_BLSTRPSTL001");
	if (GetIsObjectValid(object3)) {
		ClearAllActions();
		AssignCommand(oK33_academyguard, ActionEquipItem(object3, 4, 0));
		sub2("Gun valid", 10, 10, 4.0);
	}
	else {
		sub2("Gun not valid", 10, 10, 4.0);
	}
	DelayCommand(0.5, sub3(oK33_academyguard));
}
