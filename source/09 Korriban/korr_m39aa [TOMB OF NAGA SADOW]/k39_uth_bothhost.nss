...
void main() {
	...
	ChangeToStandardFaction(oKor39_yuthura, 1);
	sub1(3);
	object object5 = GetItemPossessedBy(OBJECT_SELF, "G_W_DBLSBR002");
	if (GetIsObjectValid(object5)) {
		object oRWeapItem = GetItemInSlot(4, OBJECT_SELF);
		if ((!GetIsObjectValid(oRWeapItem))) {
			ClearAllActions();
			ActionEquipItem(object5, 4, 0);
			sub2("Uthar Sword valid", 10, 10, 4.0);
		}
		else {
		...
		sub2("Uthar Sword not valid", 10, 10, 4.0);
	}
	object object9 = GetItemPossessedBy(oKor39_yuthura, "g_w_lghtsbr02");
	if (GetIsObjectValid(object9)) {
		object object11 = GetItemInSlot(4, oKor39_yuthura);
		if ((!GetIsObjectValid(object11))) {
			AssignCommand(oKor39_yuthura, ClearAllActions());
			AssignCommand(oKor39_yuthura, ActionEquipItem(object9, 4, 0));
	...
	object object13 = GetItemPossessedBy(oKor39_yuthura, "g_w_shortsbr02");
	if (GetIsObjectValid(object13)) {
		object oLWeapItem = GetItemInSlot(5, oKor39_yuthura);
		if ((!GetIsObjectValid(oLWeapItem))) {
			AssignCommand(oKor39_yuthura, ClearAllActions());
			AssignCommand(oKor39_yuthura, ActionEquipItem(object13, 5, 0));
	...
	SetMinOneHP(oKor39_yuthura, 0);
	DelayCommand(1.0, sub3(oPC));
	DelayCommand(1.0, AssignCommand(oKor39_yuthura, sub3(oPC)));
}
