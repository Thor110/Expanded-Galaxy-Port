...
void sub6(int intParam1) {
	sub7(OBJECT_SELF, intGLOB_1, intParam1);
}
...
void main() {
	object oPC = GetFirstPC();
	if ((((GetGlobalNumber("KOR_START_FINAL") == 1) && (sub1() == 0)) && (GetEnteringObject() == oPC))) {
		sub3("Final Started", 10, 10, 5.0);
		object oKor39_kor36 = GetObjectByTag("kor39_kor36", 0);
		AssignCommand(oKor39_kor36, ActionOpenDoor(oKor39_kor36));
		SetGlobalFadeOut(0.0, 2.0, 0.0, 0.0, 0.0);
		object oKor39_utharwynn = GetObjectByTag("kor39_utharwynn", 0);
		if ((!GetIsObjectValid(oKor39_utharwynn))) {
		...
		if ((!GetIsObjectValid(oKor39_yuthura))) {
		...
		NoClicksFor(1.2);
		DelayCommand(1.0, sub4("kor39_utharwynn", "", OBJECT_INVALID));
		DelayCommand(1.0, SetGlobalNumber("KOR_START_FINAL", 2));
		DelayCommand(1.0, sub6(1));
	}
	else {
		sub3("Final ! Started", 10, 10, 5.0);
	}
