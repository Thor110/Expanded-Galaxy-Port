	...
	if (((structGLOB_1.int13 == 4) || (structGLOB_1.int13 == 0))) {
		...
		if ((nRandom > 0)) {
			if ((((GetLevelByClass(4, OBJECT_SELF) > 0) || (GetLevelByClass(3, OBJECT_SELF) > 0)) || (GetLevelByClass(5, OBJECT_SELF) > 0))) {
				...
				sub4(("GENERIC DEBUG *************** Starting Jedi Combo " + sub50(int2)));
				...
				if (((GetLevelByClass(6, OBJECT_SELF) > 0) || (GetLevelByClass(7, OBJECT_SELF) > 0))) {
					...
					sub4(("GENERIC DEBUG *************** Starting Droid Combo " + sub50(int2)));
					SetLocalNumber(OBJECT_SELF, intGLOB_8, 1);
					SetLocalNumber(OBJECT_SELF, intGLOB_7, int2);
					return sub51(int2);
				}
					...
					sub4(("GENERIC DEBUG *************** Starting NPC Combo " + sub50(int2)));
					SetLocalNumber(OBJECT_SELF, intGLOB_8, 1);
					SetLocalNumber(OBJECT_SELF, intGLOB_7, int2);
					return sub51(int2);
				}
		...
		sub4(("GENERIC DEBUG *************** Continuing Combo " + sub50(structGLOB_1.int15)));
...
void main() {
	...
	object oK38b_door_jorak = GetObjectByTag("k38b_door_jorak", 0);
	SetLocked(oK38b_door_jorak, 0);
	if (GetIsObjectValid(oNPC)) {
		AssignCommand(oNPC, sub1(intGLOB_103, 0));
	}
	if (GetIsObjectValid(object9)) {
		AssignCommand(object9, sub1(intGLOB_103, 0));
	}
	ChangeToStandardFaction(OBJECT_SELF, 1);
	ChangeToStandardFaction(oKor38b_mekel, 2);
	object object13 = GetItemPossessedBy(OBJECT_SELF, "G_W_DBLSBR002");
	if (GetIsObjectValid(object13)) {
		ClearAllActions();
		ActionEquipItem(object13, 4, 0);
		sub2("Sword valid", 10, 10, 4.0);
	}
	else {
		sub2("Sword not valid", 10, 10, 4.0);
	}
	DelayCommand(0.5, sub3(oSpeaker));
	DelayCommand(0.5, AssignCommand(oKor38b_mekel, sub3(oKor38b_jorakuln)));
}
