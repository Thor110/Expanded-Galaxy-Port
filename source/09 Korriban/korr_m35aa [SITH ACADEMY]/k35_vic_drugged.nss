...
void main() {
	SetGlobalBoolean("KOR_TORTURE_DRUG", 1);
	object oKor35_torturer = GetObjectByTag("kor35_torturer", 0);
	object oPC = GetFirstPC();
	SetGlobalBoolean("KOR_VICTIM_UNCON", 1);
	object oK35_way_victim = GetObjectByTag("k35_way_victim", 0);
	JumpToObject(oK35_way_victim, 1);
	SetFacing(0.0);
	ApplyEffectToObject(0, EffectDeath(0, 1), OBJECT_SELF, 0.0);
	SetCommandable(1, oKor35_torturer);
	NoClicksFor(0.7);
	DelayCommand(0.5, sub1("kor35_torturer", "", OBJECT_INVALID));
}
