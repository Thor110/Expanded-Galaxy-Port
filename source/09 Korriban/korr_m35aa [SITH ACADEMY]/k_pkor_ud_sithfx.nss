...
void main() {
	...
	if ((int1 == 1001)) {
		if ((!GetIsInCombat(OBJECT_SELF))) {
			object oK35_init_grdhost = GetObjectByTag("k35_init_grdhost", 0);
			if ((!sub1(oK35_init_grdhost, intGLOB_118))) {
				ClearAllActions();
				ActionPlayAnimation(11, 0.25, 0.0);
			}
		}
	}
	else {
		if ((int1 == 1002)) {
			if ((GetLastPerceived() == oNPC)) {
			}
		}
		else {
			if ((int1 == 1003)) {
