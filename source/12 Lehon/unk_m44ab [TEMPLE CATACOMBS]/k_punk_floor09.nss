...
void main() {
	if ((GetGlobalBoolean("UNK_TILES") == 0)) {
		...
		if ((sub1(GetObjectByTag("FloorPanel09", 0)) == 1)) {
			AssignCommand(GetObjectByTag("FloorPanel09", 0), ActionPlayAnimation(200, 1.0, 0.0));
			sub2(GetObjectByTag("FloorPanel09", 0), 0);
		}
		else {
			AssignCommand(GetObjectByTag("FloorPanel09", 0), ActionPlayAnimation(201, 1.0, 0.0));
			sub2(GetObjectByTag("FloorPanel09", 0), 1);
		}
	}
