...
void sub1(object objectParam1, int intParam2) {
	object oNearestPunk_wpnpcext = GetNearestObjectByTag("punk_wpnpcext", OBJECT_SELF, 1);
	sub2(objectParam1);
	if ((GetIsObjectValid(oNearestPunk_wpnpcext) && (IsObjectPartyMember(oNearestPunk_wpnpcext) == 0))) {
		sub4(oNearestPunk_wpnpcext, intParam2);
	}
}

void main() {
	AssignCommand(GetObjectByTag("blackrakata412", 0), sub1(GetObjectByTag("blackrakata412", 0), 1));
}
