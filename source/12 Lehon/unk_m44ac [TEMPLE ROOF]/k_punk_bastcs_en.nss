...
void main() {
	...
	if ((GetIsPC(oEntering) && (!sub1(OBJECT_SELF)))) {
		sub2(OBJECT_SELF, 1);
		SetGlobalBoolean("UNK_BASTILA_CS", 1);
		sub3("entering", 5, 5, 1.0);
		DelayCommand(0.44, sub4());
		DelayCommand(0.45000002, sub5("bastilae", "", OBJECT_INVALID));
		DelayCommand(0.5, sub7(GetPartyMemberByIndex(0), GetLocation(GetObjectByTag("playerwp", 0))));
		DelayCommand(0.5, sub7(GetPartyMemberByIndex(1), GetLocation(GetObjectByTag("pm1wp", 0))));
		DelayCommand(0.5, sub7(GetPartyMemberByIndex(2), GetLocation(GetObjectByTag("pm2wp", 0))));
	}
}
