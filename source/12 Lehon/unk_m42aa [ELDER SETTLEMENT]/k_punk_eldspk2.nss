...
void main() {
	...
	if ((((GetIsPC(oEntering) == 1) && (sub1(OBJECT_SELF) == 0)) && (GetGlobalBoolean("Unk_One_Dead") == 1))) {
		sub2("setting leader 1", 5, 5, 1.0);
		sub3(SetPartyLeader((-1)));
		sub4(OBJECT_SELF, 1);
		if ((!GetGlobalBoolean("Unk_RedHostile"))) {
			AssignCommand(GetObjectByTag("unk42_redelder", 0), ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", ""));
		}
	}
}
