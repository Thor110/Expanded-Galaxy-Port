...
void main() {
	if ((GetIsPC(GetEnteringObject()) == 1)) {
		...
		SetPartyLeader((-1));
		DelayCommand(0.5, sub1(GetObjectByTag("playerwp", 0), GetObjectByTag("pm1wp", 0), GetObjectByTag("pm2wp", 0)));
		NoClicksFor(1.2);
		DelayCommand(1.0, AssignCommand(GetObjectByTag("invisp", 0), ActionStartConversation(GetFirstPC(), "unk41_holo", 0, 0, 1, "", "", "", "", "", "")));
		DelayCommand(1.5, DestroyObject(OBJECT_SELF, 0.0, 0, 0.0));
	}
}
