...
void main() {
	int int1 = sub1(10, 0);
	sub2(int1);
	SetGlobalBoolean("UNK_WG", 1);
	int int3 = 1;
	object oNearestWestroom = GetNearestObjectByTag("westroom", OBJECT_SELF, 1);
	while (GetIsObjectValid(oNearestWestroom)) {
		DelayCommand(0.5, ApplyEffectToObject(0, EffectVisualEffect(4003, 0), oNearestWestroom, 0.0));
		(int3++);
		oNearestWestroom = GetNearestObjectByTag("westroom", OBJECT_SELF, int3);
	}
}
