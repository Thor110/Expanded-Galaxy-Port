...
void main() {
	if (((sub1(OBJECT_SELF) == 0) && (GetGlobalNumber("G_FinalChoice") > 0))) {
		...
		SetPartyLeader((-1));
		sub3("firing end trigger", 5, 5, 1.0);
		if ((GetGlobalBoolean("Unk_BlackHostile") == 1)) {
			sub3("red ending", 5, 5, 1.0);
			AssignCommand(GetObjectByTag("invis", 0), ActionStartConversation(GetPCSpeaker(), "unk41_endr", 0, 0, 0, "", "", "", "", "", ""));
		}
		else {
			if ((GetGlobalBoolean("Unk_RedHostile") == 1)) {
				sub3("black ending", 5, 5, 1.0);
				AssignCommand(GetObjectByTag("invis", 0), ActionStartConversation(GetPCSpeaker(), "unk41_endb", 0, 0, 0, "", "", "", "", "", ""));
			}
		}
	}
}
