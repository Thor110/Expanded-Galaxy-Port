...
void main() {
	...
	while ((GetIsObjectValid(oAreaObject) == 1)) {
		if ((((GetTag(oAreaObject) == "tat18_tusken") || (GetTag(oAreaObject) == "tat18_tusken02")) && (GetDistanceToObject(oAreaObject) < 10.0))) {
			ChangeToStandardFaction(oAreaObject, 1);
			AssignCommand(oAreaObject, DelayCommand(1.0, sub1(oPC)));
		}
		oAreaObject = GetNextObjectInArea(OBJECT_INVALID, 1);
	}
	ChangeToStandardFaction(OBJECT_SELF, 1);
	DelayCommand(1.0, sub1(oPC));
}
