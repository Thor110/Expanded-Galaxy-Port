...
int sub12(int intParam1, int intParam2);
void sub11();
			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub13(intGLOB_46, 1));
					}
					else {
...
void main() {
	...
	sub2();
	sub11();
	ApplyEffectToObject(0, EffectDamage(50, 8, 0), OBJECT_SELF, 0.0);
	if ((sub15(GetArea(OBJECT_SELF), intGLOB_136) == 1)) {
	}
	if ((sub15(GetArea(OBJECT_SELF), intGLOB_134) == 1)) {
		effect efDamageInc = EffectDamageIncrease(8, 4096);
		ApplyEffectToObject(2, efDamageInc, OBJECT_SELF, 0.0);
	}
	if ((sub15(GetArea(OBJECT_SELF), intGLOB_135) == 1)) {
