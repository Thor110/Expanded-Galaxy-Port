			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub4(intGLOB_73, 1));
					}
					else {
	...
	if ((!sub2(intGLOB_82))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub2(intGLOB_79)) {
				...
				if (((nLocal < int4) && (!sub2(intGLOB_79)))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub2(intGLOB_80), 2.5, float3);
			if (sub2(intGLOB_73)) {
