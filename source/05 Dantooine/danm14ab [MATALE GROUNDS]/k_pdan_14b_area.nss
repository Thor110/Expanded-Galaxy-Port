...
void main() {
	...
	if ((GetIsPC(oEntering) && (GetLoadFromSaveGame() == 0))) {
		...
		if (((((GetGlobalNumber("DAN_ROMANCE_PLOT") > 0) && (GetGlobalBoolean("DAN_SAND_REFUSE") == 0)) && (GetGlobalBoolean("DAN_DIARY_DONE") == 0)) && sub3())) {
			CreateObject(64, string1, GetLocation(GetObjectByTag(("wp_" + string1), 0)), 0);
			int1 = 1;
			while ((int1 <= int2)) {
				CreateObject(1, string2, GetLocation(GetObjectByTag((("wp_" + string2) + IntToString(int1)), 0)), 0);
				(int1++);
			}
		}
