...
void main() {
	if ((((GetEnteringObject() == GetFirstPC()) && (GetGlobalNumber("DAN_PLANET_PLOT") >= 2)) && (sub1(OBJECT_SELF) == 0))) {
		sub2(OBJECT_SELF, 1);
		AssignCommand(GetObjectByTag("Deesra", 0), ActionStartConversation(GetFirstPC(), "", 0, 0, 0, "", "", "", "", "", "", 0));
	}
}
