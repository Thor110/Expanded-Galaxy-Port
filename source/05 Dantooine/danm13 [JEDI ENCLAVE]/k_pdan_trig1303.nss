...
void main() {
	...
	if ((oEntering == GetFirstPC())) {
		object oNearestDan13_WP_council = GetNearestObjectByTag("dan13_WP_council", OBJECT_SELF, 1);
		if ((GetGlobalNumber("DAN_JEDI_PLOT") == 0)) {
			sub1("dan13_zhar", "", OBJECT_INVALID);
		}
		else {
			if ((GetGlobalNumber("DAN_JEDI_PLOT") == 1)) {
				sub1("dan13_vandar", "", OBJECT_INVALID);
				RemoveJournalQuestEntry("dan_council");
			}
			else {
				if ((GetGlobalBoolean("DAN_STARMAP_DONE") && (GetGlobalBoolean("DAN_VANDAR_DONE") == 0))) {
					if (sub3()) {
						sub1("dan13_vandar", "", OBJECT_INVALID);
					}
				}
