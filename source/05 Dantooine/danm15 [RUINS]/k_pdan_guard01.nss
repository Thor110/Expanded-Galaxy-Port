...
void main() {
	...
	if (GetIsDead(oDan15_guarda)) {
		ApplyEffectToObject(0, EffectResurrection(), oDan15_guarda, 0.0);
		AssignCommand(oDan15_guarda, JumpToObject(GetPartyMemberByIndex(0), 1));
	}
	int int2 = (GetMaxHitPoints(oDan15_guarda) - GetCurrentHitPoints(oDan15_guarda));
	if ((int2 > 0)) {
		ApplyEffectToObject(0, EffectHeal(int2), oDan15_guarda, 0.0);
	}
	ChangeToStandardFaction(oDan15_guarda, 1);
	AssignCommand(oDan15_guarda, sub1(OBJECT_INVALID));
}
