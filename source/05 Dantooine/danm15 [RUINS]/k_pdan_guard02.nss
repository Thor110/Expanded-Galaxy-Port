...
void main() {
	...
	if (GetIsDead(oDan15_guardb)) {
		ApplyEffectToObject(0, EffectResurrection(), oDan15_guardb, 0.0);
		AssignCommand(oDan15_guardb, JumpToObject(GetPartyMemberByIndex(0), 1));
	}
	int int2 = (GetMaxHitPoints(oDan15_guardb) - GetCurrentHitPoints(oDan15_guardb));
	if ((int2 > 0)) {
		ApplyEffectToObject(0, EffectHeal(int2), oDan15_guardb, 0.0);
	}
	ChangeToStandardFaction(oDan15_guardb, 1);
	AssignCommand(oDan15_guardb, sub1(OBJECT_INVALID));
}
