...
void main() {
	...
	if ((((GetIsPC(oEntering) == 1) && (sub1(OBJECT_SELF) == 0)) && (GetGlobalBoolean("UNK_TEMPLEREADY") == 1))) {
		sub2(OBJECT_SELF, 1);
		if ((GetGlobalBoolean("UNK_BLACKHOSTILE") == 1)) {
			sub3("inside red loop", 5, 5, 1.0);
			sub4("unk41_rakguide", "", OBJECT_INVALID);
		}
		else {
			if ((GetGlobalBoolean("Unk_RedHostile") == 1)) {
				sub3("inside black loop", 5, 5, 1.0);
				sub4("unk41_rakguide", "", OBJECT_INVALID);
			}
		}
	}
