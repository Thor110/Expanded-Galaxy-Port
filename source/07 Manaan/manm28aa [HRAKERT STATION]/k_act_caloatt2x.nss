...
void main() {
	int int1 = 0;
	object oCaloThug = GetObjectByTag("CaloThug", int1);
	while ((GetIsObjectValid(oCaloThug) || (int1 < 5))) {
		if (GetIsObjectValid(oCaloThug)) {
			ChangeToStandardFaction(oCaloThug, 1);
			DelayCommand(0.5, AssignCommand(oCaloThug, ClearAllActions()));
			DelayCommand(0.5, AssignCommand(oCaloThug, sub1(OBJECT_INVALID)));
		}
		(int1++);
		oCaloThug = GetObjectByTag("CaloThug", int1);
	}
	ChangeToStandardFaction(GetObjectByTag("CaloNord", 0), 1);
