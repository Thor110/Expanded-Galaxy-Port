...
void sub6(string stringParam1) {
	location location1 = GetLocation(GetObjectByTag(("cut_" + stringParam1), 0));
	if ((GetIsObjectValid(GetObjectByTag(stringParam1, 0)) == 0)) {
		CreateObject(1, stringParam1, location1, 0);
	}
}
...
int sub4() {
	...
	if ((sub5(OBJECT_SELF, intGLOB_127) == 0)) {
		int1 = 1;
		sub1(OBJECT_SELF, intGLOB_127, 1);
	}
	return int1;
}
...
void main() {
	...
	if ((GetFirstPC() == oEntering)) {
		sub1(GetObjectByTag("man26_repsolshp", 0), intGLOB_118, 0);
		if ((GetGlobalNumber("K_Geno_Lorgal") != 1)) {
			DelayCommand(0.2, AssignCommand(GetObjectByTag("EnergyCage", 0), PlayAnimation(202, 1.0, 0.0)));
		}
    ...
    /*else {
      ...
      CreateItemOnObject("ebon_armband",GetFirstPC(),1,0);
		...
		if ((((sub2() >= intGLOB_156) && sub3()) && sub4())) {
			...
			oMan26ac_door31 = GetObjectByTag("man26ac_door31", 0);
			AssignCommand(oMan26ac_door31, ActionCloseDoor(oMan26ac_door31));
			sub6("man26_cutdrd01");
			sub6("man26_cutdrd02");
			sub6("man26_cutdrd03");
		...
		if (GetGlobalBoolean("MAN_SITHBASE_SEALED")) {
			DestroyObject(GetObjectByTag("man26_reptech", 0), 0.0, 0, 0.0);
			DestroyObject(GetObjectByTag("man26_repint", 0), 0.0, 0, 0.0);
			DestroyObject(GetObjectByTag("man26_sithpris", 0), 0.0, 0, 0.0);
		}
	}
}
