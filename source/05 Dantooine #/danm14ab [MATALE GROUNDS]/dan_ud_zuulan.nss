...
void sub3(object objectParam1) {
	...
	if (GetIsObjectValid(oDan_cut_droid1)) {
		AssignCommand(oDan_cut_droid1, ActionSurrenderToEnemies());
		CancelCombat(oDan_cut_droid1);
	}
	if (GetIsObjectValid(object5)) {
		AssignCommand(oDan_cut_droid2, ActionSurrenderToEnemies());
		CancelCombat(oDan_cut_droid2);
	}
}
				...
				if ((int1 == 1004)) {
					...
					if ((int1 == 1005)) {
						...
						if ((int1 == 1006)) {
							...
							int int5 = (GetMaxHitPoints(OBJECT_SELF) / 5);
							object oPC = GetFirstPC();
							object oDan_cut_zuulan = GetObjectByTag("dan_cut_zuulan", 0);
							object oDan_cut_droid1 = GetObjectByTag("dan_cut_droid1", 0);
							object oDan_cut_droid2 = GetObjectByTag("dan_cut_droid2", 0);
							if ((((nCurHP <= int5) && (sub1(OBJECT_SELF) == 0)) && (!GetIsInConversation(OBJECT_SELF)))) {
								sub2(oDan_cut_zuulan, 1);
								ChangeToStandardFaction(oDan_cut_droid1, 4);
								ChangeToStandardFaction(oDan_cut_droid2, 4);
								ChangeToStandardFaction(oDan_cut_zuulan, 4);
								sub3(oDan_cut_zuulan);
								NoClicksFor(1.2);
								DelayCommand(0.2, AssignCommand(oDan_cut_zuulan, ActionSurrenderToEnemies()));
								DelayCommand(0.4, AssignCommand(oDan_cut_zuulan, ClearAllEffects()));
								DelayCommand(1.0, AssignCommand(oDan_cut_zuulan, ActionStartConversation(oPC, "", 0, 0, 0, "", "", "", "", "", "")));
							}
						}
						else {
							if ((int1 == 1007)) {
								...
								AddJournalQuestEntry("Geno_Zuulan", 2, 0);
							}
							else {
								if ((int1 == 1008)) {
								}
