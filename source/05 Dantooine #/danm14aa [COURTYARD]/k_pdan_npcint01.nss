...
object sub3() {
	return GetObjectByTag(stringGLOB_2, 0);
}
...
void main() {
	...
	if (((GetIsPC(oEntering) && (sub1(OBJECT_SELF, intGLOB_118) == 0)) && IsNPCPartyMember(2))) {
		sub2(OBJECT_SELF, intGLOB_118, 1);
		AssignCommand(oEntering, ClearAllActions());
		SetGlobalNumber("DAN_CARTH_ZONE", 2);
		AssignCommand(sub3(), ActionStartConversation(oEntering, "", 0, 0, 0, "", "", "", "", "", ""));
	}
}
