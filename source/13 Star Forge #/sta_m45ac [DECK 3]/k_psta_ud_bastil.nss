...
void sub1(object objectParam1) {
	...
	DelayCommand(0.5, CancelCombat(objectParam1));
				...
				if ((int1 == 1004)) {
					...
					if ((int1 == 1005)) {
						...
						if ((int1 == 1006)) {
							...
							int int7 = GetCurrentHitPoints(OBJECT_SELF);
							int nMaxHP = GetMaxHitPoints(OBJECT_SELF);
							int int11 = ((nMaxHP / 2) - int7);
							if (((nCurHP <= 10) && (nGlobal == 3))) {
								sub1(oPC);
								DelayCommand(1.0, AssignCommand(GetObjectByTag("sta_bastlast_talk", 0), ActionStartConversation(OBJECT_SELF, "", 0, 0, 0, "", "", "", "", "", "", 0)));
							}
							else {
								if (((nCurHP <= 70) && (nGlobal == 2))) {
									sub1(oPC);
									DelayCommand(1.0, ActionStartConversation(oPC, "", 0, 0, 0, "", "", "", "", "", "", 0));
								}
								else {
									if (((nCurHP <= 80) && (nGlobal == 1))) {
										sub1(oPC);
										DelayCommand(1.0, ActionStartConversation(oPC, "", 0, 0, 0, "", "", "", "", "", "", 0));
									}
									else {
										sub2(((("Current=" + IntToString(nCurHP)) + " Bast=") + IntToString(nGlobal)), 10, 10, 2.0);
									}
								}
							}
						}
							...
							if ((int1 == 1007)) {
								...
								object object6 = GetPartyMemberByIndex(2);
								...
								DelayCommand(1.9, sub3(OBJECT_SELF));
								SetLocked(oK45_door_bast1, 0);
								SetLocked(oK45_door_bast2, 0);
								DelayCommand(2.0, SetGlobalFadeOut(0.0, 2.0, 0.0, 0.0, 0.0));
								AssignCommand(GetModule(), DelayCommand(3.0, sub4()));
							}
							else {
								if ((int1 == 1008)) {
								}
