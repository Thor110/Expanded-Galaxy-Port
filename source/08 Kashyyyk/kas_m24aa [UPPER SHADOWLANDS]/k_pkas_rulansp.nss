...
// Prototypes
void sub15();
int sub14(int intParam1, int intParam2);
void sub13();
	...
	if ((!sub12(intGLOB_55))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub12(intGLOB_52)) {
				...
				if (((nLocal < int4) && (!sub12(intGLOB_52)))) {
					int16 = sub14(int4, nLocal);
					nLocal = (nLocal + int16);
				}
				else {
					if ((nLocal == int4)) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub12(intGLOB_53), 2.5, float3);
			if (sub12(intGLOB_46)) {
				ActionDoCommand(sub15());
			}
			SetLocalNumber(OBJECT_SELF, intGLOB_43, nLocal);
			if ((string2 != "UNKNOWN")) {
				ActionDoCommand(sub13());
			}
		}
	}
}
...
void sub4() {
	...
	if ((sub12(intGLOB_112) || GetIsEncounterCreature(OBJECT_SELF))) {
		string string3;
		int int7 = 1;
		object object1;
		object object2;
...
void main() {
	...
	if ((GetTag(OBJECT_SELF) == "Rulan4")) {
		...
		DelayCommand(0.5, sub2(1, "kas_fake", GetLocation(GetObjectByTag("rulan_tach_wp3", 0))));
