...
void main() {
	...
	if (sub1()) {
		...
		object object5 = GetPartyMemberByIndex(0);
		if ((oEntering == GetFirstPC())) {
			object oEnd_flame01 = CreateObject(64, "end_flame01", GetLocation(GetObjectByTag("end_wp_flame01", 0)), 0);
			DestroyObject(oEnd_flame01, 1.0, 0, 0.0);
			ApplyEffectToObject(0, sub4(), oEntering, 0.0);
			SetGlobalNumber("END_TRASK_DLG", 9);
			ActionStartConversation(oNPC, "end_trask01", 0, 0, 1, "", "", "", "", "", "");
		}
		else {
			AssignCommand(oNPC, ClearAllActions());
			AssignCommand(oNPC, ActionStartConversation(object5, "end_cut02", 0, 0, 1, "", "", "", "", "", ""));
		}
	}
}
