...
*/
void main() {
//StartNewModule("end_m01ab");
          DelayCommand(8.0, SignalEvent(GetObjectByTag("end_bridgesith1", 0), EventUserDefined(20)));
					DelayCommand(12.0, SignalEvent(GetObjectByTag("end_bridgesith2", 0), EventUserDefined(20)));
          ...
          CreateObject(1, "end_bridgesith1", GetLocation(GetObjectByTag("wp_end_bridgesith1", 0)), 0);
          CreateObject(1, "end_bridgesith2", GetLocation(GetObjectByTag("wp_end_bridgesith2", 0)), 0);
...
/*
	...
	if (GetGlobalBoolean("END_BRIDGE_SEALED")) {
		...
		if ((GetGlobalNumber("END_ROOM8_DEAD") >= 2)) {
			...
			if (((sub5() < intGLOB_165) && (sub6(GetFirstPC()) == 2))) {
				sub8(intGLOB_165);
				sub9();
			}
			else {
