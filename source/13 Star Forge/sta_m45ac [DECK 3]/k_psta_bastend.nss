...
void main() {
	...
	int nMaxHP = GetMaxHitPoints(oSta_bastila);
	int nCurHP = GetCurrentHitPoints(oSta_bastila);
	int nMaxFP = GetMaxForcePoints(oSta_bastila);
	int nCurFP = GetCurrentForcePoints(oSta_bastila);
	effect efHeal = EffectHeal((nMaxHP - nCurHP));
	effect effect3 = EffectHealForcePoints((nMaxFP - nCurFP));
	ApplyEffectToObject(0, efHeal, oSta_bastila, 0.0);
	ApplyEffectToObject(0, effect3, oSta_bastila, 0.0);
	int int9 = 0;
	float float1 = 0.0;
	object oSta_bastila_heal = GetObjectByTag("sta_bastila_heal", 0);
	object oSta_beam_top1 = GetObjectByTag("sta_beam_top1", int9);
	object oSta_beam_bottom1 = GetObjectByTag("sta_beam_bottom1", int9);
	while ((oSta_beam_top1 != OBJECT_INVALID)) {
		DelayCommand(float1, ApplyEffectToObject(1, EffectBeam(2038, oSta_beam_top1, 1, 0), oSta_bastila_heal, 2.0));
		(int9++);
		oSta_beam_top1 = GetObjectByTag("sta_beam_top1", int9);
		oSta_beam_bottom1 = GetObjectByTag("sta_beam_bottom1", int9);
	}
	ApplyEffectToObject(1, EffectBeam(2038, oSta_bastila_heal, 1, 0), oSta_bastila, 2.0);
	ApplyEffectToObject(1, EffectVisualEffect(1019, 0), oSta_bastila, 3.0);
	ChangeToStandardFaction(oSta_bastila, 1);
	SetMinOneHP(oSta_bastila, 0);
	object oPC = GetFirstPC();
	AssignCommand(oSta_bastila, sub1(oPC));
}
