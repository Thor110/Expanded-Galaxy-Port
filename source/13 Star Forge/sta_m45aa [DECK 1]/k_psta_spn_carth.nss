...
void main() {
	...
	object oNearestCarth = GetNearestObjectByTag("Carth", OBJECT_SELF, 1);
	object oNearestBastila = GetNearestObjectByTag("Bastila", OBJECT_SELF, 1);
	object oPC = GetFirstPC();
	if (((GetGlobalBoolean("STA_MALAK_DEAD") && (sub1(OBJECT_SELF, intGLOB_1) == 0)) && (GetEnteringObject() == oPC))) {
		sub2(OBJECT_SELF, intGLOB_1, 1);
		if ((((GetGlobalNumber("G_FinalChoice") == 2) || (GetGender(oPC) == 1)) || (GetGender(oPC) == 0))) {
			object oSta_way_carth = GetObjectByTag("sta_way_carth", 0);
			location location1 = GetLocation(oSta_way_carth);
			sub3(1, "sta_carth", location1);
		}
	}
}
