						...
						if ((int1 == 1006)) {
							...
							if ((int1 == 1007)) {
								...
								if ((GetJournalEntry("kas23_mainwookplot") < 57)) {
									AddJournalQuestEntry("kas23_mainwookplot", 57, 0);
								}
								else {
									AddJournalQuestEntry("kas23_mainwookplot", 87, 0);
								}
								CancelCombat(oPC);
								SetPartyLeader(0xFFFFFFFF);
								AssignCommand(oKas25_ritualtalk, ActionStartConversation(oPC, "kas25_xbeast_01", 0, 0, 1, "", "", "", "", "", ""));
							}
							else {
								if ((int1 == 1008)) {
								}
