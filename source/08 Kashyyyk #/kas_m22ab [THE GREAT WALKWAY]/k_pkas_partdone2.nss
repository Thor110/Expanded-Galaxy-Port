...
void sub1(int intParam1, object objectParam2) {
	if (((intParam1 == 1) || (intParam1 == 0))) {
		sub2(GetArea(objectParam2), intGLOB_2, intParam1);
	}
	return;
}
...
void main() {
	...
	object oKas_party_wp2 = GetWaypointByTag("kas_party_wp2");
	sub1(1, OBJECT_SELF);
	if ((GetIsObjectValid(oMission) == 1)) {
		AssignCommand(oMission, ClearAllActions());
		AssignCommand(oMission, JumpToObject(oKas_party_wp2, 1));
	}
	else {
		if ((GetIsObjectValid(oCarth) == 1)) {
			AssignCommand(oCarth, ClearAllActions());
			AssignCommand(oCarth, JumpToObject(oKas_party_wp2, 1));
		}
		else {
			if ((GetIsObjectValid(oBastila) == 1)) {
				AssignCommand(oBastila, ClearAllActions());
				AssignCommand(oBastila, JumpToObject(oKas_party_wp2, 1));
			}
			else {
				if ((GetIsObjectValid(oJuhani) == 1)) {
					AssignCommand(oJuhani, ClearAllActions());
					AssignCommand(oJuhani, JumpToObject(oKas_party_wp2, 1));
				}
				else {
					if ((GetIsObjectValid(oCand) == 1)) {
						AssignCommand(oCand, ClearAllActions());
						AssignCommand(oCand, JumpToObject(oKas_party_wp2, 1));
					}
					else {
						if ((GetIsObjectValid(oHK47) == 1)) {
							AssignCommand(oHK47, ClearAllActions());
							AssignCommand(oHK47, JumpToObject(oKas_party_wp2, 1));
						}
					}
				}
			}
