...
int sub5(int intParam1, int intParam2);
			...
			if ((nRandom == 3)) {
				...
				if ((((nRandom == 4) || (nRandom == 5)) && (GetRacialType(OBJECT_SELF) != 5))) {
					if ((GetGender(OBJECT_SELF) == 0)) {
						...
						ActionDoCommand(sub6(intGLOB_73, 1));
					}
					else {
	...
	if ((!sub4(intGLOB_82))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub4(intGLOB_79)) {
				...
				if (((nLocal < int4) && (!sub4(intGLOB_79)))) {
					int16 = sub5(int4, nLocal);
					nLocal = (nLocal + int16);
				}
				else {
					if ((nLocal == int4)) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub4(intGLOB_80), 2.5, float3);
			if (sub4(intGLOB_73)) {
				ActionDoCommand(sub7());
			}
			SetLocalNumber(OBJECT_SELF, intGLOB_70, nLocal);
			if ((string2 != "UNKNOWN")) {
				ActionDoCommand(sub3());
			}
		}
	}
}
