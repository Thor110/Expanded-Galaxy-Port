	...
	if ((!sub3(intGLOB_55))) {
		...
		if ((GetDistanceToObject2D(object3) <= 2.5)) {
			...
			if (sub3(intGLOB_52)) {
				...
				if (((nLocal < int4) && (!sub3(intGLOB_52)))) {
		...
		if (GetIsObjectValid(GetWaypointByTag(string9))) {
			...
			ActionForceMoveToObject(object3, sub3(intGLOB_53), 2.5, float3);
			if (sub3(intGLOB_46)) {
...
void main() {
	...
	AssignCommand(OBJECT_SELF, sub1(intGLOB_48, 1));
	AssignCommand(OBJECT_SELF, sub2());
}
