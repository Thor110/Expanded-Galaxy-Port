...
void main() {
	...
	while (GetIsObjectValid(oAreaObject)) {
		if ((!IsObjectPartyMember(oAreaObject))) {
			if ((GetStandardFaction(oAreaObject) == 4)) {
				ChangeToStandardFaction(oAreaObject, 1);
				DelayCommand(0.5, AssignCommand(oAreaObject, sub1(OBJECT_INVALID)));
			}
			else {
				if (((GetTag(oAreaObject) == "assaultdrd") && (GetStandardFaction(oAreaObject) == 2))) {
					sub79("CHANGING DROID FACTION", 5, 5, 5.0);
					DelayCommand(1.0, ChangeToStandardFaction(oAreaObject, 6));
					DelayCommand(0.5, AssignCommand(oAreaObject, sub1(OBJECT_INVALID)));
				}
			}
		}
