// Globals
	int intGLOB_1 = 0;
	int intGLOB_2 = 1;
	int intGLOB_3 = 2;
	int intGLOB_4 = 3;
	int intGLOB_5 = 4;
	int intGLOB_6 = 5;
	int intGLOB_7 = 6;
	int intGLOB_8 = 7;
	int intGLOB_9 = 8;
	int intGLOB_10 = 9;
	int intGLOB_11 = 10;
	int intGLOB_12 = 11;
	int intGLOB_13 = 12;
	int intGLOB_14 = 13;
	int intGLOB_15 = 14;
	int intGLOB_16 = 15;
	int intGLOB_17 = 16;
	int intGLOB_18 = 17;
	int intGLOB_19 = 18;
	int intGLOB_20 = 19;
	int intGLOB_21 = 1100;
	int intGLOB_22 = (-6);
	int intGLOB_23 = (-5);
	int intGLOB_24 = (-4);
	int intGLOB_25 = (-2);
	int intGLOB_26 = (-1);
	int intGLOB_27 = 0;

// Prototypes
void sub2(object objectParam1, int intParam2);
void sub1(string stringParam1, int intParam2, int intParam3, float floatParam4);

void sub2(object objectParam1, int intParam2) {
	ActionForceMoveToObject(objectParam1, intParam2, 1.0, 30.0);
	ActionDoCommand(SetCommandable(1, OBJECT_SELF));
	SetCommandable(0, OBJECT_SELF);
}

void sub1(string stringParam1, int intParam2, int intParam3, float floatParam4) {
	if ((!ShipBuild())) {
		AurPostString(stringParam1, intParam2, intParam3, floatParam4);
	}
}

void main() {
	object oZaalbar;
	object oLev40_partyesc = GetObjectByTag("lev40_partyesc", 0);
	int int1 = 0;
	while ((int1 < 7)) {
		switch (int1) {
			case 0:
				oZaalbar = GetObjectByTag("cand", 0);
				break;
			case 1:
				oZaalbar = GetObjectByTag("jolee", 0);
				break;
			case 2:
				oZaalbar = GetObjectByTag("juhani", 0);
				break;
			case 3:
				oZaalbar = GetObjectByTag("hk47", 0);
				break;
			case 4:
				oZaalbar = GetObjectByTag("t3m4", 0);
				break;
			case 5:
				oZaalbar = GetObjectByTag("mission", 0);
				break;
			case 6:
				oZaalbar = GetObjectByTag("zaalbar", 0);
				break;
		}
		if (GetIsObjectValid(oZaalbar)) {
			sub1((GetTag(oZaalbar) + " IS FLEEING"), 5, (5 + int1), 5.0);
			DelayCommand(14.0, DestroyObject(oZaalbar, 0.0, 0, 0.0));
			DelayCommand((int1 * 1.0), AssignCommand(oZaalbar, sub2(oLev40_partyesc, 1)));
		}
		(int1++);
	}
	ActionPauseConversation();
	ActionWait(9.0);
	ActionResumeConversation();
}

