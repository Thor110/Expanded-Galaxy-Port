...
void main() {
	...
	object oLev40_partycelldoor = GetObjectByTag("lev40_partycelldoor", 0);
	if (((GetIsPC(object1) || (object1 == OBJECT_SELF)) && (!sub1(OBJECT_SELF)))) {
		sub2(OBJECT_SELF, 1);
		sub3("ELEVATOR = 1", 5, 5, 1.0);
		SetGlobalNumber("LEV_ELEVATOR", 1);
		SetGlobalBoolean("LEV_RESCUE", 1);
		if ((!GetIsOpen(oLev40_partycelldoor))) {
			AssignCommand(oLev40_partycelldoor, ActionOpenDoor(oLev40_partycelldoor));
		}
		AssignCommand(GetObjectByTag("bastila400", 0), ActionStartConversation(object1, "", 0, 0, 1, "", "", "", "", "", ""));
	}
}
