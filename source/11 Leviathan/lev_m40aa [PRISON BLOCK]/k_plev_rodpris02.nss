...
void sub3(object objectParam1, int intParam2) {
	...
	if (GetIsObjectValid(oNearestPlev_wpnpcext)) {
		sub2(oNearestPlev_wpnpcext, intParam2);
	}
}
...
void main() {
	object oNearestLev40_guard1 = GetNearestObjectByTag("lev40_guard1", OBJECT_SELF, 1);
	if (GetIsObjectValid(oNearestLev40_guard1)) {
		sub1("ATTACKING", 5, 5, 5.0);
		ChangeToStandardFaction(OBJECT_SELF, 2);
		sub2(oNearestLev40_guard1, 1);
	}
	else {
		sub1("LEAVING", 5, 5, 5.0);
		sub3(OBJECT_SELF, 1);
